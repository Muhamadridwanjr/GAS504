"""FastAPI entry point for gas-chart-service."""
...
async def lifespan(app: FastAPI):
    ...
    app.state.orchestrator = ChartOrchestrator(cache)
    logger.info("%s ready", settings.SERVICE_NAME)
    yield
    await cache.close()

app = FastAPI(title="gas-chart-service", description="Chart data orchestrator with templates and favorites.", version="1.0.0", lifespan=lifespan, docs_url="/docs", redoc_url="/redoc")
