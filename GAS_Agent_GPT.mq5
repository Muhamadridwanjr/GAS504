...
#property description "GAS AI Agent: GPT-5.4 — General Reasoning"
...
input int      MIN_CONFIDENCE   = 72;           // Minimum confidence to execute
input double   LOT_SIZE         = 0.01;
input int      MAGIC_NUMBER     = 20260002;
...
int OnInit()
{
   ...
   agentStatus = "ACTIVE";
   Print("GAS Agent GPT-5.4 initialized. Poll every ", POLL_INTERVAL, "s");
...
void PollSignal()
{
   ...
   string payload = StringFormat(
      ...
      "\"model\":\"gpt\",\"min_confidence\":%d,\"max_trades\":2}",
...
void ParseAndAct(string json, string symbol)
{
   ...
   bool ok = false;
   if(lastSignal == "BUY") {
      ok = trade.Buy(LOT_SIZE, symbol, price, sl, tp, "GAS-GPT-Agent");
   } else if(lastSignal == "SELL") {
      ok = trade.Sell(LOT_SIZE, symbol, price, sl, tp, "GAS-GPT-Agent");
   }
