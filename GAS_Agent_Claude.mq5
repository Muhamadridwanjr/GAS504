//+------------------------------------------------------------------+
//|  GAS AI Agent — Claude Sonnet 4.6                               |
...
#property description "GAS AI Agent: Claude Sonnet 4.6 — High Accuracy"
...
input int      MIN_CONFIDENCE   = 70;           // Minimum confidence to execute
input double   LOT_SIZE         = 0.01;
input int      MAGIC_NUMBER     = 20260001;
...
int OnInit()
{
   ...
   agentStatus = "ACTIVE";
   Print("GAS Agent Claude initialized. Poll every ", POLL_INTERVAL, "s");
...
void PollSignal()
{
   ...
   string payload = StringFormat(
      ...
      "\"model\":\"claude\",\"min_confidence\":%d,\"max_trades\":2}",
...
void ParseAndAct(string json, string symbol)
{
   ...
   bool ok = false;
   if(lastSignal == "BUY") {
      ok = trade.Buy(LOT_SIZE, symbol, price, sl, tp, "GAS-Claude-Agent");
   } else if(lastSignal == "SELL") {
      ok = trade.Sell(LOT_SIZE, symbol, price, sl, tp, "GAS-Claude-Agent");
   }
