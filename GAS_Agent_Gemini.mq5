//+------------------------------------------------------------------+
//|  GAS AI Agent — Gemini Pro 3.1                                  |
//|  Golden AI Strategy — gasstrategyai.xyz                         |
//|  EA polls signal from GAS API every 15s, auto-executes orders   |
//+------------------------------------------------------------------+
#property copyright "Golden AI Strategy"
#property link      "https://gasstrategyai.xyz"
#property version   "1.00"
#property description "GAS AI Agent: Gemini Pro 3.1 — Structure Analysis"
...
input string   TRADE_STYLE      = "intraday";   // scalping | intraday | swing
input int      MIN_CONFIDENCE   = 68;           // Minimum confidence to execute
input double   LOT_SIZE         = 0.01;
input int      MAGIC_NUMBER     = 20260003;
input int      POLL_INTERVAL    = 15;           // Seconds between signal checks
...
int OnInit()
{
   ...
   agentStatus = "ACTIVE";
   Print("GAS Agent Gemini Pro 3.1 initialized. Poll every ", POLL_INTERVAL, "s");
...
void PollSignal()
{
   ...
   string payload = StringFormat(
      ...
      "\"model\":\"gemini\",\"min_confidence\":%d,\"max_trades\":2}",
...
void ParseAndAct(string json, string symbol)
{
   ...
   bool ok = false;
   if(lastSignal == "BUY") {
      ok = trade.Buy(LOT_SIZE, symbol, price, sl, tp, "GAS-Gemini-Agent");
   } else if(lastSignal == "SELL") {
      ok = trade.Sell(LOT_SIZE, symbol, price, sl, tp, "GAS-Gemini-Agent");
   }
